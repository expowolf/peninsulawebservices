  ...
  return (
    <section className="py-20 md:py-28 bg-white">
      <div className="container-x">
        <p className="text-xs uppercase tracking-[0.28em] font-semibold text-slate-500 mb-3 text-center">
          Get Started
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {tiers.map(({ name, sub, desc, cta, href, dark }) => (
            ...
            >
              <h3 className={`font-display text-2xl font-bold ${dark ? '' : 'text-slate-900'}`}>{name}</h3>
              <p className={`text-sm mt-2 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>{sub}</p>
              <p className={`mt-5 leading-relaxed ${dark ? 'text-slate-300' : 'text-slate-600'}`}>{desc}</p>
              <a
                href={href}
